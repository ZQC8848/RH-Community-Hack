...
| A video shows only its first frame - no motion, no sound - while `isPrepared` and `isPlaying` both report true and nothing errors | Unity's `VideoPlayer` needs ~18s on this machine to deliver a first picture, and the code reset the decoder every loop, so the startup never finished | [2026-08-28-video-shows-first-frame-only-and-never-advances.md](2026-08-28-video-shows-first-frame-only-and-never-advances.md) |
