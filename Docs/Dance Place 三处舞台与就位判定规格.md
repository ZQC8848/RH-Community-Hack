---
状态: v2（舞台已 prefab 化，编辑器实测，未在头显验证，2026-09-01）
...
| | 份数 | 内容 |
|---|---|---|
| **布景**（`DanceStage.prefab`） | ×3 | 穹顶、圆形地板、视频屏 + **自带 VideoPlayer**、舞者 ×3、传送锚点 + 灯柱、世界面板 |
...
**未就位时没有任何面板**——这是刻意的。寻路靠的是三根灯柱（§5.2），不是面板也不是海报。
...
**一、从外面能看进去。** 反向绕序下近侧半球被背面剔除，所以站在 Stage 1 能望见 Stage 2、3 的内部——远处那两根灯柱不会被自己的穹顶挡住。

**二、穹顶没有 collider。** 这是必需的，不是省事：有 collider 的话传送弧线会打在自家墙上，跨舞台传送直接废掉。

> 实测：从 Stage 1 手部高度（0, 1.2, 0）朝 Stage 2 的灯柱打射线，`RaycastAll` 只有 **1 个命中**，就是灯柱本身（23.71 m）。两侧穹顶壁、两块地板都不在路上。
