...
#### 5.2.1 三个高度只差 1 厘米，而且**是相对舞台的**

```
舞台根 +0.03   Facing Arrow      朝向箭头
舞台根 +0.02   Standing Marker   站位圆板   ← 压在白线之上（它得能看见）
舞台根 +0.015  白线                         ← 就挤在这一格
舞台根 +0.01   Dome Floor        穹顶地板
舞台根  0      地面
```

> ⚠️ **`StageTimeline.height` 是相对每个 stop 的，不是世界坐标。** v7 里它一度写成绝对的 0.015；后来场景整体被抬到 y=0.1，**白线就沉到地面下面 85 mm，完全看不见了，而且没有任何东西提示为什么**。相对之后地面怎么升降都跟着走。
