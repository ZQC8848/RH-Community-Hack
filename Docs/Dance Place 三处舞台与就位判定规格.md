---
状态: v8（新增序章；时代道具入场，编辑器实测，未在头显验证，2026-09-05）
...
```
舞台根 +0.03   Facing Arrow      朝向箭头
舞台根 +0.02   Standing Marker   站位圆板
舞台根 +0.01   Dome Floor        穹顶地板   ← 把白线盖住
舞台根 +0.005  白线                         ← 上下各只有 5 mm
舞台根  0      地面
```

**白线穿在每处舞台的地板下面**，所以它只在舞台之间的空地上可见，一进圆盘范围就被盖住——圆盘因此读起来像线上的一个个站点。

实测：正上方俯视时地板范围内一点都不透，180 米外贴地平视也没有深度冲突。

> ⚠️ **`StageTimeline.height` 是相对每个 stop 的，不是世界坐标。** 它一度写成绝对的 0.015；后来场景整体被抬到 y=0.1，**白线就沉到地面下面 85 mm，完全看不见了，而且没有任何东西提示为什么**。相对之后地面怎么升降都跟着走。
>
> **调到 +0.01 以上，线就爬到地板上面去了。**
...
### 14.4 溶解 shader —— **只作用在球体上**

`Assets/Shaders/DomeDissolve.shader`，**只有穹顶用它**（`_Cull` 2 / `_Sweep` 0.55 / `_NoiseScale` 8）。

> **圆形地板不溶解，也不消失。** 它是玩家脚下的地面——在头显里看着地板从自己脚下碎开，和看着房间从头顶打开，是完全不同的两件事，后者舒服得多。穹顶散尽之后留下的是一块亮色圆盘，标记着这里曾是一处舞台，白线正好从它上面穿过。
>
> 所以地板已经**换回 `Universal Render Pipeline/Unlit` 不透明材质**：它不再需要 alpha clip，而一块这么大的 alpha-test 表面在一体机上值得避免——它会让画在它后面的东西全部失去 early-Z。

**⚠️ 溶解完成时关掉的是球体的 `Renderer`，不是 `Dome` 这个 GameObject。** 地板是 `Dome` 的子物体，关掉 GameObject 会把地板、地板的 `MeshCollider`、以及玩家脚下的地面一起带走。
...
剧本在时间线出现**之前**还有一段：玩家出生在全黑空间，叙述者讲 IFEL 的故事，物件随她点名逐个浮现在四周，然后「周围物件缓缓滑向远处，地面时间线倒数回 1964 年」。

**序章不是一处舞台。** 它在线之外，自己的锚点上（世界坐标 `(-45, 0.1, -20)`，离 Stage 1 有 49 米，远超它 20 米的舞者半径，所以背后不会有穹顶亮起来），期间白线长度为 0。

### 15.1 组成

| | |
|---|---|
| `PrologueCue` | 一拍：说话人、字幕、**语音 clip**、时长、这一拍浮现哪些道具、是否全部退场 |
| `PrologueDirector`（挂 `Play Controller`） | 走 cue 列表，结束后交给 `TimelineDirector` |
...
序章结束时调 `TimelineDirector.Begin()`，它把 `index` 设成 **-1** 然后进 `Grow` 阶段。于是**现有循环原封不动地**做完了开场：

```
Grow    线从 0 长到第一处舞台   ← 剧本「时间线从脚下延伸出去」
Settle  停一拍
Travel  淡出 → 把玩家搬上去 → 淡入
Dwell   正常循环开始
```

没有一条专门的开场路径需要跟主循环保持同步。`LengthAtStop(-1)` 返回 0 就是为这个。

> `TimelineDirector.autoStart` 必须**关掉**，否则它和序章会同时驱动玩家。新增了 `Idle` 状态和 `IsWaitingToBegin`。

### 15.4 ⚠️ 字幕面板是懒跟随的，这是设计不是偷懒

焊在头上的文字是最可靠的致晕方式之一——它永远不停，眼睛没法落在上面。所以面板**在你转头时不动**，只有视线偏离超过 `deadZoneDegrees`（18°）才开始跟，而且是平滑跟随不是瞬移。序章里玩家本来就要四处转头看浮现的物件，这一条决定了字幕能不能读。

同样**不能用 `Screen Space - Overlay` 的 Canvas**——它在头显里根本不渲染，和 `ScreenFade` 是同一个坑。
...
舞台 prefab 多了一个空的 `Props` 挂载点，`DancePlace.props` 指着它。**实例把自己的道具挂在下面**——所以它们是 added GameObject 而不是组件 override，`take` 仍然是唯一的 component override。

`Props` 跟舞者**共用同一个距离半径**开关（20 m + 3 m 滞后）。这些模型一个就是六位数三角形，不可能全程常驻。

| 道具 | 舞台 | 剧本依据 |
|---|---|---|
| `hitsville` | Stage 2 - 1964 | 「前方是 Hitsville USA / 摩城唱片的 3D 模型」 |
| `CRTV` | Stage 3 - 1984 | 「一台巨大的电视围绕视频出现」 |
| `minicity` | Stage 3 - 1984 | 「念到不同城市时，出现一个迷你城市模型」 |
| `tech-totem` | Stage 4 - 2016 | 1984→2016 的音乐技术蒙太奇，落在它引向的年代 |

### 16.1 摆放踩到的两个坑

**一、世界空间的枢轴偏移不能直接加到 `localPosition` 上。** 第一版这么做了，而序章根节点和每一处舞台都是转过角度的，结果 5G 塔跑到了玩家头顶。正确做法是全程走父物体自己的空间：`parent.InverseTransformVector(wantedWorld - bounds.center)`。

**二、别假定模型的最长轴是 Y。** `tech-totem` 的网格本地范围是 `0.003 × 0.003 × 0.010`——它是**躺着的**。按高度归一化把它拉成了 12.5 米长。改用最长边归一化。

> 电视也往后挪过：它是个 6×4.2×5.9 的厚实机身，原来的位置把 z=2.5 的视频屏整个包进了机身里。现在近侧面在 z=2.8，框住屏幕而不是盖住它。
